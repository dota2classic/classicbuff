...
import { renderParticipantImage, renderParticipantLabel } from "./text";
import {
...
export class BracketsViewer {
  ...
  private participants!: TournamentBracketParticipantDto[];
  ...
  public render(data: ViewerData, config?: Partial<Config>): void {
    ...
    this.config = {
      ...
      showLowerBracketSlotsOrigin:
        config && config.showLowerBracketSlotsOrigin !== undefined ? config.showLowerBracketSlotsOrigin : true,
      highlightParticipantOnHover:
        config && config.highlightParticipantOnHover !== undefined ? config.highlightParticipantOnHover : true
    };
    ...
    data.stages.forEach(stage =>
      this.renderStage(root, {
        ...data,
        stages: [stage],
        matches: data.matches.filter(match => match.stageId === stage.id)
      })
    );
  }
  ...
  private renderStage(root: HTMLElement, data: ViewerData): void {
    ...
    const matchesByGroup = splitBy(data.matches, "groupId");
   ...
   */
  private renderRoundRobin(root: HTMLElement, stageName: string, matchesByGroup: TournamentBracketMatchDto[][]): void {
    const container = dom.createRoundRobinContainer();

    let groupNumber = 1;

    for (const groupMatches of matchesByGroup) {
      const groupId = groupMatches[0].groupId;
      const groupContainer = dom.createGroupContainer(groupId, lang.getGroupName(groupNumber++));
      const matchesByRound = splitBy(groupMatches, "roundId");

      let roundNumber = 1;

      for (const roundMatches of matchesByRound) {
        const roundId = roundMatches[0].roundId;
  ...
  private renderElimination(
    ...
    stageName: string,
    type: StageType,
    matchesByGroup: TournamentBracketMatchDto[][]
  ): void {
    root.append(dom.createTitle(stageName));

    if (type === "single_elimination") this.renderSingleElimination(root, matchesByGroup);
    else this.renderDoubleElimination(root, matchesByGroup);
  }
   ...
   * @param matchesByGroup A list of matches for each group.
   */
  private renderSingleElimination(root: HTMLElement, matchesByGroup: TournamentBracketMatchDto[][]): void {
    const hasFinal = matchesByGroup[1] !== undefined;
    this.renderBracket(root, splitBy(matchesByGroup[0], "roundId"), lang.getRoundName, "single-bracket");
   ...
   * @param matchesByGroup A list of matches for each group.
   */
  private renderDoubleElimination(root: HTMLElement, matchesByGroup: TournamentBracketMatchDto[][]): void {
    const hasLoserBracket = matchesByGroup[1] !== undefined;
    const hasFinal = matchesByGroup[2] !== undefined;

    this.renderBracket(
      root,
      splitBy(matchesByGroup[0], "roundId"),
    ...
    if (hasLoserBracket)
      this.renderBracket(root, splitBy(matchesByGroup[1], "roundId"), lang.getLoserBracketRoundName, "loser-bracket");
  ...
  private renderBracket(
    root: HTMLElement,
    matchesByRound: TournamentBracketMatchDto[][],
    roundName: RoundName,
    bracketType: BracketType,
    connectFinal?: boolean
  ): void {
    const groupId = matchesByRound[0][0].groupId;
   ...
   */
  private renderFinal(finalType: FinalType, matches: TournamentBracketMatchDto[]): void {
    ...
    for (let roundIndex = 0; roundIndex < finalMatches.length; roundIndex++) {
      ...
      const roundContainer = dom.createRoundContainer(
        finalMatches[roundIndex].roundId,
   ...
   */
  private createRanking(matches: TournamentBracketMatchDto[]): HTMLElement {
    ...
    for (const prop in item) {
      ...
      if (prop === "id") {
        ...
        if (participant !== undefined) {
          const cell = dom.createCell(participant.id);
  ...
  private createMatch(
    ...
    const team2 = this.createTeam(match.opponent2 || null, originHint, matchLocation, roundNumber);

    if (label) opponents.append(dom.createMatchLabel(label, lang.getMatchStatus(match.status)));

    if (label && match.games.length > 0) {
      const box = dom.createMatchLabel(`BO${match.games.length}`, `BO${match.games.length}`);
      box.classList.add("best-of-x");
      opponents.append(box);
    }

    opponents.classList.add(lang.getMatchStatus(match.status));
   ...
   */
  private createTeam(
    participant: ParticipantResultDto | null,
    originHint: OriginHint,
    matchLocation?: BracketType,
    roundNumber?: number
  ): HTMLElement {
    ...
    if (participant === null) containers.name.innerText = "BYE";
    else this.renderParticipant(containers, participant, originHint, matchLocation, roundNumber);
  ...
  private renderParticipant(
    ...
    if (found) {
      renderParticipantImage(containers.name, found);
      renderParticipantLabel(containers.name, found);
