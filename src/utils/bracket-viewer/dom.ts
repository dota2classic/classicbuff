...
import { ParticipantResultDto } from "../../api/back/models";
...
export function createParticipantContainer(id: number | null): HTMLElement {
  ...
  if (id !== null) team.setAttribute("data-participant-id", id.toString());
...
export function setupWin(
  ...
  if (participant.result && participant.result === "win") {
    ...
    if (participant.score === undefined) resultContainer.innerText = abbreviations.win;
  }
}
...
export function setupLoss(
  ...
  if ((participant.result && participant.result === "loss") || participant.forfeit) {
    ...
    if (participant.forfeit) resultContainer.innerText = abbreviations.forfeit;
    else if (participant.score === undefined) resultContainer.innerText = abbreviations.loss;
  }
}
