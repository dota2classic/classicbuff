...
import { ParticipantResultDto, TournamentBracketMatchDto } from "../../api/back/models";
...
export function findRoot(selector?: string): HTMLElement {
  ...
  if (!root.classList.contains("bracket-viewer")) throw Error("The selected root must have a `.bracket-viewer` class.");
...
export function getOriginAbbreviation(matchLocation: string, skipFirstRound: boolean, roundNumber?: number): string {
  if (skipFirstRound && matchLocation === "lower-bracket" && roundNumber === 1) return abbreviations.seed;

  if (matchLocation === "lower-bracket" || matchLocation === "final-group") return abbreviations.position;
 ...
 */
export function getRanking(matches: TournamentBracketMatchDto[], formula?: RankingFormula): Ranking {
  formula = formula || ((item: RankingItem): number => 3 * item.wins + 1 * item.draws + 0 * item.losses);

  const rankingMap: RankingMap = {};

  for (const match of matches) {
    processParticipant(rankingMap, formula, match.opponent1 || null, match.opponent2 || null);
    processParticipant(rankingMap, formula, match.opponent2 || null, match.opponent1 || null);
  }
...
function processParticipant(
  ...
  current: ParticipantResultDto | null,
  other: ParticipantResultDto | null
): void {
